...
from ben10_content import DAILY_EVENTS, ITEM_CATALOG, LORE_CHAPTERS, NPCS, SIDE_JOBS
...
def pick_random_entry(entries: list, seed: int) -> dict:
    if not entries:
        return {}
    random.seed(seed)
...
def read_lore(profile: dict) -> None:
    UI.section("Lore Archive")
    entry = pick_random_entry(LORE_CHAPTERS, profile["Day"] + profile["Level"])
    print(wrap(entry))
    profile["Experience"] += 1


def meet_npc(profile: dict) -> None:
    UI.section("Meet a Local")
    npc = pick_random_entry(NPCS, profile["Day"] * 3 + profile["Experience"])
    name = npc.get("name", "Unknown")
    print(wrap(f"You meet {name}. {npc.get('hook', '')}"))
    profile["Reputation"]["Bellwood"] += 1
    profile["Experience"] += 2


def browse_inventory(profile: dict) -> None:
    UI.section("Inventory Check")
    if not profile["Inventory"]:
        print("Your inventory is empty.")
        return
    for idx, item in enumerate(profile["Inventory"], start=1):
        print(f"{idx}. {item}")


def use_item(profile: dict) -> None:
    UI.section("Use Item")
    if not profile["Inventory"]:
        print("No items to use.")
        return
    choice = prompt_choice("Select an item", profile["Inventory"] + ["Cancel"])
    if choice == "Cancel":
        return
    if choice in profile["Inventory"]:
        profile["Inventory"].remove(choice)
        profile["Resources"]["Energy"] = min(20, profile["Resources"]["Energy"] + 2)
        profile["Resources"]["Resolve"] = min(12, profile["Resources"]["Resolve"] + 1)
        print(f"You use {choice} and feel refreshed.")


def browse_catalog(profile: dict) -> None:
    UI.section("Supply Catalog")
    entry = pick_random_entry(ITEM_CATALOG, profile["Level"] * 7 + profile["Day"])
    print(wrap(f"Featured item: {entry.get('name', 'Unknown')}"))
    print(wrap(entry.get("effect", "")))


def take_side_job(profile: dict) -> None:
    UI.section("Side Job")
    job = pick_random_entry(SIDE_JOBS, profile["Day"] + profile["Completed Missions"])
    reward = job.get("reward", 5)
    print(wrap(job.get("summary", "")))
    profile["Resources"]["Credits"] += reward
    profile["Experience"] += 3
    print(f"You earn {reward} credits.")
...
def play_game(profile: dict) -> None:
    ...
    while True:
        ...
        if action == "Travel":
            ...
            show_map(world)
        elif action == "Meet a Local":
            meet_npc(profile)
        elif action == "Read Lore":
            read_lore(profile)
        elif action == "Browse Catalog":
            browse_catalog(profile)
        elif action == "Use Item":
            use_item(profile)
        elif action == "Check Inventory":
            browse_inventory(profile)
        elif action == "Take Side Job":
            take_side_job(profile)
        elif action == "End Day":
