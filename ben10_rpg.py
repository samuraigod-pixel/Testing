...
def random_npc_interaction(profile: dict) -> None:
    UI.section("Local Encounter")
    npc = random.choice(ben10_content.NPCS)
    print(wrap(npc))
    profile["Reputation"]["Bellwood"] += 1
    profile["Experience"] += 1
    print("You gain local goodwill and 1 XP.")


def scavenge_for_item(profile: dict) -> None:
    UI.section("Scavenge")
    item = random.choice(ben10_content.ITEMS)
    profile["Inventory"].append(item)
    profile["Experience"] += 1
    print(wrap(f"You found: {item}"))
    print("You gain 1 XP.")


def trigger_life_event(profile: dict) -> None:
    UI.section("Life Event")
    event = random.choice(ben10_content.EVENTS)
    print(wrap(event))
    profile["Resources"]["Energy"] = max(0, profile["Resources"]["Energy"] - 1)
    profile["Experience"] += 2
    print("The day shifts in unexpected ways. You gain 2 XP.")
...
def play_game(profile: dict) -> None:
    ...
    while True:
        ...
        elif action == "Meet Local":
            random_npc_interaction(profile)
        elif action == "Scavenge":
            scavenge_for_item(profile)
        elif action == "Life Event":
            trigger_life_event(profile)
